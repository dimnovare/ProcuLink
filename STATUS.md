...
| **F** | PDF ingestion (`PdfPig`) | **Implemented — text→LLM extraction is the PRIMARY path (`feat/pdf-llm-extraction` merging to `main`, 780 tests green); regex parser is fallback; Azure DI removed; scanned/image-only SHIPPED via Phase 2 AI vision (review-flagged, no Dockerfile change); self-hosted OCR = Phase 3 (pending)** |
