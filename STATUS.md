...
> - **W1 ✅ SHIPPED + deploy-verified** (`28a72f4`, from cont.3 tail) — `OrderService` 1618→124-line internal facade delegating to OrderIngestion/Query/Resolution/Transform + OrderServiceShared; same 12-dep ctor + `IOrderService` so DI/Worker/tests untouched; **verbatim moves**, adversarially reviewed **SAFE-TO-MERGE**; 1028 tests. Background `/health`+`/health/ready` poll after deploy = exit 0 (healthy on Railway).
> - **UI/UX #6 admin-nav gating ✅ SHIPPED** — BE `e41f6e6` (new bodyless `GET /api/admin/access` reusing the class-level `[AdminOnly]` gate → 204 admin / 403 non-admin; no DB hit; allowlist never serialized) + FE `ac16373` (`BridgeSidebar` filters the Admin link via a `checkAdminAccess` probe; gated on `queryEnabled`, 5-min staleTime, no retry, default-hidden while loading, `true` in mock). **UX-only** — the `/admin` page + every `/api/admin` endpoint still re-gate server-side, so it can only show/hide a link, never grant access. 17 AdminControllerTests green + FE build clean; no e2e depends on the link.
> - **Part C launch checklists reconciled** (`216b3fa`) — the generic MVP/Production templates had drifted from the per-item tracker. Marked code-done items with evidence (Inbound nav already behind `INBOUND_ENABLED`; ingress idempotency §1.1.C; pool cap B8; stuck-requeue R2; worker alert B7; exception handler P1-2; tenant unify R1; R2/GDPR erase W3; version pins B1-B4). **Confirmed the SSRF DNS-rebinding TOCTOU is already CLOSED** — `OutboundRequestGuard.CreateGuardedHttpHandler()` `ConnectCallback` re-resolves + rejects private/metadata IPs at connect time. Remaining `[ ]` are genuine **founder/ops/sales** actions (real-PO-to-real-supplier, Stripe live-key swap, monitored mailbox, alert destinations) — tagged as such.
> - **§1.1.E retention sweep → [x]** — `DataRetentionService` + `DataRetentionSweepJob` are code-complete + safe-by-default OFF; per-order R2 erase = W3; enablement is founder-env like every config-gated capability.
> - **Postgres RLS (§2.5) → FINAL-DEFERRED** (not done, by design) — the audit *itself* classifies it "redesign-later / post-revenue, NOT launch-blocking" (§2.8). It is the **highest prod-breaking risk remaining** (an unset session-var → zero rows would break Hangfire sweeps + migrations) and **cannot land green safely** without a real-Postgres testcontainer two-org harness the suite lacks + Hangfire/migration/sweep role exemptions; app-level `.Where(OrgId==…)` already enforces 100% isolation. Per the founder's own "no new bugs is the tiebreaker → drop any that can't land green safely," it is correctly dropped, not rushed. Ready plan: `WAVE_D_BACKEND_REMAINING.md`.
> - **Net:** every `[ ]` *code* item in `PROD_LAUNCH_AUDIT.md` is now `[x]` or audit-deferred `[—]`; the remaining open items are founder/ops/sales or roadmap. **The audit's actionable code surface is CLOSED.** Full `dotnet test ProcuLink.slnx` = **1029 green, 0 failures** (224 Transform + 452 Infra + 353 Api). main HEAD `216b3fa`; FE `main` `ac16373` (build clean).

## Where we are: **2026-06-07 (cont.3) — Wave D backend: W2/W3/W4 SHIPPED + verified live; W1 + Postgres RLS handed off**
...
> - **Structural items (founder: build-now + auto-merge-when-green) — 3 of 4 SHIPPED + adversarially reviewed + merged:** **W4** ✅ (Redis-nonce + headers, above); **W2** ✅ `OrderStatusMachine` explicit state machine + centralized Redeliver guard (`39e069e`, 1028 tests); **W3** ✅ per-order GDPR erase `IDataErasureService` + admin endpoint (`1424cfa`) — the adversarial review caught + fixed **2 real bugs pre-merge** (a `order_confirmation_lines` RESTRICT-FK abort that would have made every erase fail on prod after deleting R2; a confirmation PO-data + R2 leak).
> - **DEFERRED to a dedicated session — W1 (OrderService 1618-LOC decompose behind the `IOrderService` facade) + Postgres RLS.** Both are the audit's own "pure risk / zero customer value / redesign-later, do-NOT-refactor-pre-launch" items; W1 has shared private helpers (a ~full-session faithful move) and RLS needs a session-var connection interceptor + a real-Postgres two-org test harness (EF InMemory can't test RLS) — rushing either at the end of a 200+-step session is the exact new-bug risk the **"no new bugs" tiebreaker** (founder's own) forbids. Ready-to-execute plans + the flagged-counterproductive list (W6 api-client split [collides w/ active FE chips], W5 retry-consolidate [refactors correct code], denormalize/partition, Neon-pooler+DataRetention-enable [env], Postmark-signature [needs CF Worker], SchemaFingerprints/phantom-migration [dangerous pre-launch], social proof [needs data], EDIFACT [licence], cross-org/i18n/PEPPOL [roadmap]) → **`docs/strategy/WAVE_D_BACKEND_REMAINING.md`**. **Live `- [x]` trackers in both audit docs.** main HEAD `c923d91`; continue W1/RLS in a fresh isolated worktree off `main`.
...
> **Full handoff for the next chip: `docs/strategy/NEXT_CHIP_HANDOFF.md`** (read first), then `docs/strategy/LAUNCH_EXECUTION_PLAN.md`, then `docs/strategy/PROD_LAUNCH_AUDIT.md`.
>
> - **Waves 0-6 ✅ SHIPPED + deploy-verified.** (0) honesty/dead-CTA fixes. (1) correctness: Npgsql pool ceiling (`BuildPooledConnectionString`, lazy read), ingress idempotency, AI chunking guard, stuck-order requeue + additive `requeue_count` migration (`20260607090000_AddOrderRequeueCount`, applied on prod Neon — needed an explicit `HasColumnName` mapping in `ProcuLinkDbContext`). (2) security P1: SSRF guard on ERP connectors (Erply/Directo via named "delivery" HttpClient + CGNAT/benchmark ranges), global exception handler + ProblemDetails, `azp` validation kept, CORS wildcard-subdomain removed, tenant-resolution unified, billing-iface, rate-limit policies APPLIED (Stripe webhook deliberately spared). (3) reliability: Worker Sentry + heartbeat alert (`WorkerHealthAlertJob`/`SentryWorkerAlertSink`), deep health (`/health` fast liveness vs `/health/ready` readiness w/ DB+storage+migration checks), data-retention sweep (dormant), migrate fail-loud. (4) billing: `StripeConfiguration.AppInfo`, Stripe test-mode QA passed (4 prices active incl. Distributor checkout session), `docs/deployment/stripe-go-live-runbook.md` (founder does the live swap). (5/6) UX: pricing 6→3 tiers + ROI slider + progressive disclosure, Distributor upsell in BillingSection, OnboardingWizard a11y, `next` pinned `15.5.18` + engines, explicit `Ai:OpenAI:ExtractionModel`, downloadInvoice blob fix, removed invented ZONE_CONF/formats→Configurable. Adversarial 2nd-reviewer caught **4 real issues pre-ship** (ERP SSRF, dead rate-limit policies, unbounded provision dict, path-traversal Ordinal); **zero regressions reached prod**.
> - **Wave 7 (full prod test) — FORMAT LEG ✅ DONE 2026-06-07.** All 6 upload formats parse live on prod (CSV=ready/3, XLSX[ClosedXML]=ready/3, cXML=pending_review/2, UBL=pending_review/1, EDIFACT=pending_review/3, X12-via-true-`.x12`=pending_review/3); PDF proven earlier (real Markit PX89 via OpenAI `gpt-4o-mini`). Recipe + fixtures: memory `project-live-format-testing` + `.live-fixtures/` (browser-JS upload via `window.Clerk` token; `file_upload` tool rejects non-share paths; Chrome stuck at 1920 viewport → no mobile screenshots; openpyxl-xlsx fails on prod .NET → use ClosedXML).
> - **Wave 7 — DELIVERY + INGRESS legs ✅ DONE 2026-06-07 (live, founder's session).** **HTTP/webhook delivery** test-fired through the real `HttpDeliveryDispatcher` + `OutboundRequestGuard` to a real webhook.site URL → `{success:true,responseCode:200}` + payload captured. **FULL loop to `delivered`**: CSV order `1b02ec9a` → transform → `ready_to_deliver` → redeliver → **`delivered`**, transformed PO XML (resolved SUP-001/002/003) captured at webhook.site; configs deleted after (baseline restored). **API-key ingress** (Zapier/Make/REST): created `plk_` key → `/ingress/{slug}/ping` 200 → `POST /ingress/{slug}/orders` created order `92411272` (pending_review, 2 lines) → key revoked. SFTP/FTPS/SMTP delivery + SFTP/IMAP/S3 ingress + inbound-email already proven via committed live-endpoint tests + a real prod email (2026-06-05). Recipe in memory `project-live-format-testing`.
> - **Wave 7 REMAINING:** admin UI walk (already verified live 2026-06-06), Sentry/PostHog event verification (needs dashboard access — founder-side), mobile-viewport visual QA (Chrome stuck at 1920 here — QA via code/DOM). ERP delivery = mock REST per founder.
> - **Wave 8 — clean-env regression gate ✅ GREEN (2026-06-07 cont.):** killed stale processes, `dotnet test ProcuLink.slnx` = **988 green** (224+413+351), `bun run build` clean; prod `/health` 200 + `/health/ready` Healthy. **SPF/DKIM/DMARC ✅ COMPLETE** — DMARC `_dmarc.proculink.eu` `v=DMARC1; p=none; rua=mailto:dim.novare+dmarc@gmail.com; fo=1` added by a parallel chip, **verified resolving live** (CF API + 8.8.8.8 + 1.1.1.1); SPF+DKIM were already correct (Resend `send.` subdomain + `resend._domainkey`). **Consolidated ops runbook ✅** `docs/deployment/launch-operations-runbook.md` (`03b24fa`). Still pending: reconcile CLAUDE.md "211 tests" → 988.
> - **Wave D — design-primitive page migration ✅ SHIPPED + DEPLOYED + VERIFIED LIVE (frontend `004c7d0`).** Migrated the **12 in-app pages with real bespoke shells** (drafts; operations/{health,connectors,exceptions,webhooks}; admin; library/{buyers,templates,standards}; inbound/{invoices,asns}; settings) onto `PageShell/PageHeader/Card/MobileListRow` + `UnifiedStatusBadge` + `Button` + design tokens. (The other 10 routes are thin wrappers w/ no page-level markup.) Standardized **in-app primary CTAs → brand-green** (founder decision; buyer-blue stays accent for rails/nav/links). Orchestration: migrate→adversarial-verify pipeline (24 agents) + a 2nd report-only behavior-diff review (12 agents) → **zero behavior regressions** (caught+fixed pre-commit: health smart-quote corruption, a Card-padding table break, dropped mobile rules, dangling imports; only no-op left = a mobile `title` tooltip). Build green (48 routes); live-verified `/operations/health` renders + connectors "Add" btn = `rgb(46,142,58)`. **Core daily-driver components behind the thin wrappers (InboxView/SpineReview/UploadWorkbench/SupplierDock*) deliberately NOT migrated this pass** (just live-QA'd; mass-refactor = regression risk) — careful follow-up.
> - **Wave D — backend items DEFERRED to a focused isolated-worktree effort** (OrderService god-object split behind `IOrderService`; order status enum+transition table; Postgres RLS; typed-DTO layer; split `api-client.ts`; EmailPolling indexed flag; R2/DB GDPR-delete; Redis only >1 replica). Reason: backend `main` is a **moving target under concurrent chips** (advanced `4a405f1`→`15f11ab` mid-session) + these are exactly the audit's "do-NOT-refactor-before-launch" items; per the **"no new bugs" tiebreaker** they need branch/worktree isolation + adversarial review, not a rushed merge 2 days pre-launch. Plan stands; spin a dedicated chip.
> - **Live launch-readiness sweep ✅ (founder opened all dashboards 2026-06-07)** — `docs/deployment/2026-06-07-launch-readiness-verification.md`. Railway API+Worker Online + env complete; Vercel prod=HEAD; Sentry capturing (all 22 issues STALE — every path re-exercised live); PostHog ingesting live; Stripe browser-hard-blocked (verified via API/env). **DMARC added** (`p=none`, verified resolving) → SPF+DKIM+DMARC complete.
> - **Public-site audit ✅ (22-agent workflow, adversarially verified)** — `docs/audit/2026-06-07-launch-site-audit.md`. 15 confirmed (0 P0/4 P1/5 P2/6 P3); availability+links+API-hygiene clean. **3 P1s FIXED + verified live (frontend `1b9f896`):** honest ROI footnote (dropped false "pilot customer measurements"), pricing tiers now SSR-crawlable (`[hidden]` not conditional-mount; disclosure still works), app security headers (X-Content-Type-Options/Referrer-Policy/X-Frame-Options/Permissions-Policy) + metadataBase. Remaining (routed to next chip): per-page SEO metadata+canonical, API HSTS/nosniff, app CSP, og polish, Sentry stale-resolve, Postmark log level.
> - **Frontend launch polish ✅ DONE + VERIFIED LIVE (frontend `2d7ab38`)** — knocked out the audit's remaining frontend items: per-page SEO `metadata` + self-`canonical` + `og:url` on `/how-it-works`, `/formats`, and `/pricing` (client page → new server `layout.tsx` wrapper); app **CSP `Content-Security-Policy: frame-ancestors 'self'`** (modern anti-clickjacking, in sync with X-Frame-Options); **og-image.png 1.99 MB → 119 KB** (resized 2400→1200×630 matching the declared OG dims, full quality, visually identical); homepage hero **4+/5+/4 → 9/6/6** (verified against the `/formats` SoT: 9 live inbound formats · 6 outbound · 6 delivery channels — offer⇔works honest). **Verified live:** CSP header present, og = 121,592 B, hero renders 9/6/6, all 3 canonicals resolve. STILL next-chip: API HSTS/nosniff (Railway/edge-side), FULL app CSP (script/style/connect — needs Clerk/Stripe/PostHog/Sentry testing), per-page SEO on the remaining marketing pages, Sentry stale-issue resolve, Postmark webhook log level.
> - **✅ RESOLVED — design migration shipped clean (was the "broken uncommitted WIP").** The 12-page design-primitive migration is now **committed + deployed + verified live** on `main` (`004c7d0`), build green, behavior-reviewed clean. **⚠️ COORDINATION:** a **parallel "ui-parity" worktree** was doing the SAME migration and left it broken (curly quotes). That worktree is now **SUPERSEDED — discard it; do NOT commit its version over `main`** (it would clobber the verified-clean pages with the broken curly-quote versions). If that chip is still running, stop it / have it rebase onto `004c7d0`.
> - **Founder decisions:** deep refactors DO-NOW; Stripe test-now + founder does live swap; ERP=mock; Cloudflare write-token provided for DNS. **Secrets held in memory only (never committed/printed), rotate post-launch.**

## Where we are: **2026-06-06 (latest) — final scoped + P3 items shipped; VERIFIED LIVE IN PRODUCTION (authed screens, real data)**

> Last build pass + a real deployed-site QA. Frontend `main` `e820f3a` (build clean); backend `main` `07e0e9b`. Both pushed + deployed (Vercel + Railway healthy).
> - **Final code pass ✅ SHIPPED (4 parallel worktree agents + integration fix).** SpineReview **2-col tablet (md–xl) layout** (tablets no longer drop to the phone accordion); PoMappingEditor reads the **shared standards catalog** + `StandardsFieldPopover` + mobile lineage cue; Inbox **keyboard nav (j/k/Enter) + column-visibility menu + chrome-preserving skeleton**; Settings SFTP/S3 use the **shared SettingsGroup shell** + brand-green Save, direction radiogroup got **arrow-key/roving-tabindex a11y**, connector empty-state leads with the working path. Integration fix: extracted `SettingsGroup`+`primaryGreenButton` to `@/components/settings/SettingsPrimitives` — removed a page↔component import cycle AND a Next "page may not export non-default symbols" build error (`next build` caught what `tsc` missed).
> - **LIVE PRODUCTION QA ✅ (drove the deployed site via the founder's logged-in Chrome).** Verified on `proculink.eu` with REAL data: **Inbox** — live action counts ("1 need review · 8 failed" header + chip badges, previously mock-only), the **failure-bucket** Failed chip (8 = all 5 failure statuses), the renamed "Ready to send" chip, honest "Unknown buyer" label, mobile cards at 390px, Columns menu, Exceptions/Health nav. **`/admin`** — full area working: **MRR €149 "Reconciled with Stripe (€149)"** (DB compute + Stripe reconcile agree), ARR/active/trialing/new-orgs/trial→paid, the 10-org customers table, the "Adjust limits / extend pilot" override + "Create invoice". (Founder's Clerk id is correctly allowlisted — page loaded, not the refuse view.) **`/settings`** — the "How do you use ProcuLink?" direction control + "About this workspace · FIXED". Deploy health: api/landing/pricing/sign-in all 200; admin endpoints 401 unauthed (gate fail-closed).
> - **Backlog: effectively cleared.** Only genuinely-marginal P3 remains in the audit doc.

## Where we are: **2026-06-06 — full P1/P2/P3 audit sweep + live-e2e gate fix + resolve header-edit shipped; live-verified**

> Continuation. Backend `main` `3219a40` (**876 tests**); frontend `main` `e5b49e9` (`bun run build` clean; **mock e2e suite green**). Both pushed.
> - **Audit P1 + P2 (+ co-located P3) sweep ✅ SHIPPED (7 parallel worktree agents).** Frontend (`92ffde5`): **SpineReview** (backend grand-total everywhere, all line items visible, discoverable+scoped shortcuts, dialog/toast a11y, severity-colored notices, auto-refresh while in-flight, honest mapping preview); **Settings** (ingress endpoint/slug row, removed Zapier/Make 404 links, proactive SFTP/S3 gate + loading/error, default-supplier dead-end, styled confirms, email validation, currency/region "fixed"); **Dashboard** (API-error branch vs onboarding hero, "Dashboard" H1, honest labels incl. "Delivery success rate" + "based on latest 100", canonical stage badges); **Buyers+Mappings** (real columns, Mappings selection prompt, removed never-saved Description + decorative export options); **Shell+Upload** (dead workspace switcher→static badge, Help opens new tab, dropzone a11y + drag accept-filter, dropped premature "Transform" step, 6 new `loading.tsx`, mobile breadcrumb/demo-badge, palette label). Backend (`3219a40`): org `slug` in settings response + real 30-day supplier-health window.
> - **Runtime verification ✅ (drove the tests):** `dotnet test ProcuLink.slnx` = **876 green**; `bun run build` clean; **mock e2e suite green** (47 specs: 40 pass + 7 live-only skipped; the historically-flaky sample-order routing test de-flaked via a retry-click; 5 stale "Upload & send" matchers updated to the renamed "Upload & review"). The **live QA-bypass e2e** (local API+Worker booted + driven) is blocked by a PRE-EXISTING (`bf1058f`, 2026-06-03) `queryEnabled = isApiMockMode || clerkReady` gate that starves data queries in QA-bypass LIVE mode — **not a regression from this round**; documented dev-infra follow-up (needs a frontend QA-bypass signal to run live e2e).
> - **Follow-up pair ✅ SHIPPED (the two items flagged above are now DONE):** Backend `a3e8cf3` (**887 tests**): the resolve endpoint accepts + persists edited header fields `orderDate`/`buyerName`/`currency` (written to BOTH the `buyer_name` column AND CanonicalJson; 400-validation; header-only resolves allowed). Frontend `c957e01`: a `useQueriesEnabled()` hook + `isQaBypass` (`NEXT_PUBLIC_QA_BYPASS_AUTH`, no-op in prod) applied across 11 gate sites + Playwright config — **this unblocks the live QA-bypass e2e**; and **SpineReview date/buyer/currency editing re-enabled** (wired through the resolve call). `bun run build` + `tsc` clean.
> - **Live e2e — VERIFIED working end to end.** With the gate fix, the live e2e (booted local API+Worker, QA-bypass) now drives upload → navigates to the preview → the Worker **parses the CSV into 3 lines** (`pending_review`) and `GET /api/orders/{id}/mapping-preview` returns those 3 lines (confirmed via API). The previously-disabled upload button + starved queries are gone. The `live-po-loop` spec's final 30s assertion can still time out *only* because parse latency exceeds 30s when one machine runs API+Worker+next-dev+Chromium at once — confirmed ENVIRONMENTAL (the data is correct), not a code defect.
> - **Remaining:** long-tail P3 polish in the audit doc (e.g. MagicMappingPreview shows the "no lines" empty copy briefly during an in-progress parse before its 1.5s poll fills it — minor).

## Where we are: **2026-06-06 — pricing overhaul (#1–#4) + full UI/UX audit + audit P0 fixes shipped both repos**

> Continuation of the founder-feedback round. Backend `main` `e31fc0f` (**869 tests green**); frontend `main` `b0aadb6` (`bun run build` clean). Both pushed (Railway + Vercel redeploying).
> - **Pricing #1–#4 + hardening ✅ SHIPPED (billing review clean).** #1 **NEVER-BLOCK** a paid order — over the monthly cap orders still process and bill **€0.50/order overage** via a Stripe invoice item; idempotency is **period-keyed** (`{orgId}:{periodStart:O}` on a unique `(org_id,billing_key)` index + a Stripe IdempotencyKey) so a re-issued invoice can't double-bill; Pilot-expired still read-only. #2 Integration cap 1000→**1500** (monotonic €/order) + delivery/ingestion **channels decoupled** (Webhook/Email/SFTP/S3 gate moved Integration→Growth; on ALL paid plans). #3 setup fee folded into self-serve tiers, €500/supplier only Enterprise/complex. #4 per-org **admin limit/trial overrides** (`POST /api/admin/organisations/{id}/limits`, `[AdminOnly]`, trial-days capped ≤3650). Frontend: non-blocking overage notice (LimitBanner now Pilot-only), 1500/decouple/fee copy, **admin Adjust-limits modal**. **No new Stripe products** (overage = dynamic invoice items); additive migration.
> - **UI/UX audit ✅ DONE (11 parallel agents).** 152→**79 findings (6 P0·27 P1·31 P2·15 P3)** in `docs/audit/2026-06-06-ui-ux-audit.md`. Dominant theme: dead/decorative controls + staged data leaking to real users (offer⇔works).
> - **Audit 6 P0s + quick-wins ✅ FIXED (3 parallel agents).** P0 unauth bulk-send → existing `redeliverOrder`; upload dead controls + the FALSE "send without review" warning removed + "Upload & review" rename + accept-list trimmed; Exceptions/Health added to nav + Cmd+K + `LAUNCH_CORE_HREFS`; SpineReview inline header edits made **read-only** (backend resolve DTO has no header fields — see gap) + AI **Accept now really resolves** a line (Send unblocks); global Validation Rules reframed as a **non-gating catalog**; reduced-motion SMIL fixed; fake metrics removed ('Pipeline healthy · 12/min', 'alert to MK'); direction labels completed; honest billing-faq/terms/help copy (never-block + 1500 + channels-on-any-paid).
> - **Audit P1 batch — live-data + exception loop ✅ SHIPPED (5 agents).** Backend `b844bdc` (**874 tests**): inbox "Failed" filter now spans all 5 failure statuses (`failed/transform_failed/delivery_failed/delivery_dead_letter/rejected_by_supplier`); exception **Reconcile** now auto-resolves when the cause clears and won't resurrect a resolved row (no more reappearing "resolved"). Frontend `8089165` (build clean): inbox **live action counts** (`getOrdersSummary` — "N need review · N failed" + chip badges now in LIVE mode) + filter-aware empty state; supplier list columns + Mappings tab wired to live `getDeliveryConfig`/`getSupplierMappings` (dead "—" columns dropped honestly; per-row fetch capped at 50); LaneDrawer real recent-deliveries + wired "Connection settings" button; exceptions list "Resolve"→**"Open order"** (closing the operator loop with the Reconcile fix). This closes audit levers #2 (live = as informative as mock) and #4 (exception→resolution).
> - **Admin access ✅ enabled in Railway** — `Admin__UserIds=user_3EVIvVz0hTJVcTOQytjyLWQTCKX` + `Admin__Emails=redacted@example.invalid` (open `/admin` after the API redeploy).
> - **Deferred / follow-ups:** (a) backend resolve DTO needs header fields to re-enable SpineReview header editing (read-only is the honest stopgap); (b) live "Failed" inbox chip needs a `failedBucket` (backend + api-client) to span all 5 failure statuses; (c) audit **P1/P2/P3 backlog** (mostly mock-only data not wired to live APIs + a11y/consistency) — a next batch; (d) runtime QA of authed screens (admin modal, overage notice, direction relabel, mobile) once redeployed; (e) **Stripe live-mode swap (June 9)** is the only gate to real billing.

## Where we are: **2026-06-06 — founder-feedback BUILD round: extractor party-swap fixed+live-verified · mobile responsive shipped · admin backend shipped (security-clean) · Distributor self-serve**

> Continuation of the founder-feedback round — the scoped items are now BUILT + merged. Handover: `docs/handoff/2026-06-06-founder-feedback-handover.md`.
> - **Item 1 — extractor buyer/supplier swap ✅ FIXED + LIVE-VERIFIED.** Rewrote `OpenAiPdfOrderExtractor` SystemPrompt to assign roles from document labels (PO: buyer = issuer / "ordered by" / "bill to"; supplier = recipient / "supplier" / "vendor"; inverted for invoices), explicitly NOT from the familiar system-customer name; buyer≠supplier enforced. Committed gated live test (`PROCULINK_LIVE_AI_TESTS=1`) RUN with the real key on `gpt-4o-mini`: **6/6 pass** — Danfoss/REDACTED-PARTY/ABB fixtures (Markit stays supplier) AND the real 22-PDF corpus (every doc buyer≠supplier, numbers verbatim). Backend `main` `32b29d1`, **830 tests green**.
> - **Item 2 — mobile responsive ✅ SHIPPED.** Notifications full-width sheet; inbox filter row + card separation + buyer→supplier rail (stacks vertically, honest "Unknown buyer"); order-detail header no longer overlaps the PO title (stacked action bar, validate button wraps); IMAP header; dashboard density. Frontend `main` `59eb9d3`, `bun run build` clean.
> - **Item 3 — owner/admin backend ✅ SHIPPED (adversarial security review CLEAN, 0 holes).** `/api/admin/{overview,organisations,invoices}` behind an env-allowlist `[AdminOnly]` gate (fail-closed; `Admin__UserIds`/`Admin__Emails`, accepts user-id OR email); DB-computed MRR + Stripe reconcile; one-off Stripe invoicing. 27 security tests (non-admin→403, empty-allowlist→403). **Frontend `/admin` pages IN PROGRESS.** ⚠️ Founder must set `Admin__UserIds` (prod Clerk user-id) in Railway to gain access.
> - **Item 4 — Distributor ✅ SELF-SERVE (was already done in Stripe; docs lied).** Verified live via the Stripe API: Distributor product + `€1,499/mo` + `€14,928/yr` prices ACTIVE (TEST mode — same as every tier; live-swap is the June-9 gate); backend `CreateCheckoutSessionAsync` already mapped `distributor`→price. Flipped frontend `isCheckout:true` + "Upgrade to Distributor" (`0ee27ae`); promoted `Stripe:DistributorPriceId` to a REQUIRED prod key. The "TODO: create the Stripe product" note was STALE.
> - **Item 5 — "support both directions" (founder decision) DESIGNED.** Per-org `OrderDirection` (Outbound default | Inbound). Key finding: the data model is already direction-agnostic (buyer=issuer/supplier=recipient), so v1 = one Organisation column + one settings GET/PUT + a frontend `useOrderDirection` label source-of-truth relabelling the rail/CTAs; defers the Supplier→Counterparty entity remodel. 3 UX questions pending founder before build.

## Where we are: **2026-06-06 — founder-feedback round: real-PO tests RUN, pricing copy fixed; mobile + admin area scoped**
...
> - **Pricing ✅ FIXED + pushed** — the real issue was the **missing Distributor card** (the footer named it but `hidden:true` hid it), NOT the fee. **Distributor is now SHOWN on `/pricing`** as a contact-sales tier (`hidden:false`, `cta:"Contact sales"`; `isCheckout:false` so no broken Checkout until a Stripe Distributor product exists). It is the ICP tier (Baltic IT distributors — the real-PO test confirmed Markit IS a distributor) and fills the €999→€2,500 ladder gap. The €500×3/€150 fee model is REAL and stays (an earlier mis-fix that retired it was reverted). 6 tiers → clean 3×2 grid. **TODO: create the Stripe Distributor product + `DistributorPriceId` for self-serve.**
...
**Shipped (Phase 1):** the brittle regex `PdfOrderParser` + paid Azure Document Intelligence are replaced by **text→LLM structured extraction** as the PRIMARY PDF path. PdfPig extracts the digital text layer; an OpenAI extractor structures it into the canonical `ParsedOrder` (strict JSON schema — 5 header + 6 line fields; the LLM never emits a supplier item code, resolved downstream). **Anti-hallucination validation:** every emitted number must appear verbatim in the source text, and qty×unit-price must reconcile with the stated line amount; suspect lines are flagged "needs review" so they surface in `/operations/exceptions` instead of delivering blind. The regex `PdfOrderParser` is now the deterministic FALLBACK only (no OpenAI key / offline / extraction fails or low-confidence). New config key `Ai:OpenAI:ExtractionModel` (falls back to `Ai:OpenAI:MappingModel`, then `gpt-5-mini`); extractor is a safe no-op without a key. **Azure Document Intelligence removed entirely** (`Azure.AI.DocumentIntelligence` package + `AzureDocumentIntelligenceOcrService` + all `Ocr:Azure:*`/`Ocr__Azure__*` keys gone); the `IDocumentOcrService` + `NoOpOcrService` seam is kept; as of Phase 3 (below) it can now be wired to the self-hosted `RapidOcrDocumentOcrService` (opt-in via `NoEgressOcr:Enabled`), defaulting to the no-op when that flag is unset. **767 backend tests green** (built TDD; `dotnet build ProcuLink.slnx` clean, zero Azure refs; frontend `bun run build` clean). **Merged to `main` both repos** (backend `feat/pdf-llm-extraction` + the `OpenAiEmailBodyOrderExtractor` snake_case binding fix; frontend `feat/pdf-llm-copy`) — pure C# + docs, **no EF migration / no Dockerfile / no native dep**, so the Railway + Vercel auto-deploys are additive and safe. Prod `appsettings.Production.json` already sets `Ai:Provider=openai` + `MappingModel=gpt-4o-mini` and the API key is in Railway, so the extractor **activates in prod on `gpt-4o-mini`** (the model the live smoke verified). **Two adversarial multi-agent review passes** (find → 2-vote verify): 7 findings fixed in-branch — space-merge false-flags, **EU/Baltic space-grouped thousands** (`1 250,00`), 3-decimal ambiguity, positional line numbers, `double→decimal` overflow guard, input-text cap, `Guid.Empty` fail-closed, + the email-extractor snake_case binding — and 4 deferred/documented (document-wide presence gap; PdfPig DoS bound — pre-existing/platform; per-org data-residency — compliance; cancellation — intentional). **Live smoke verified** end-to-end against the real OpenAI API (`gpt-4o-mini`): a text PO extracted cleanly with zero false review flags. **Phase 2 (vision for scanned PDFs) + Phase 3 (self-hosted no-egress OCR) + Phase 4 (enrichment + invoice classifier) all shipped** (see below).
...
**Phase 4 ✅ SHIPPED 2026-06-05:** canonical enrichment (supplier name, sub/tax/grand totals, payment terms, per-line line_amount/tax_rate/delivery_date) + PO-vs-invoice `document_type` classification. Records gained DEFAULTED params (zero blast radius); migration `AddOrderEnrichmentFields` = 9 additive NULLABLE columns; an `invoice` classification forces `pending_review` + a `ClassifiedAsInvoice` audit so an invoice on the PO path is never silently delivered as a PO.

**Phase 2 SHIPPED (scanned via AI vision, review-flagged):** scanned / image-only PDFs (no text layer) are now supported via an AI **vision fallback**. When PdfPig finds no text, the system rasterizes the leading pages (**PDFtoImage + SkiaSharp**, both permissive; self-contained native assets — **no Dockerfile / system-package change**; verified loading on the Debian `aspnet:8.0` base via a Docker probe) and extracts via the vision-capable OpenAI model using the same strict schema. Live-verified end to end (image-only PDF → vision → structured order on `gpt-4o-mini`). **Honest caveat:** vision has no text layer to verify numbers against, so **every line from a scanned PDF is flagged for human review** (it surfaces in `/operations/exceptions` / order review) — assisted, not silent/auto-delivered; text-based PDFs remain the high-confidence primary path. A scanned PDF the vision model still can't read (illegible) fails with "This PDF looks scanned or image-only — we couldn't extract any text." Privacy: the vision path sends rasterized page **images** to OpenAI (same EU-residency / DPA / zero-retention considerations as the text path). **Phase 3 SHIPPED (self-hosted no-egress OCR, opt-in):** "no data leaves your environment" OCR is now AVAILABLE as an opt-in capability via `RapidOcrDocumentOcrService` (RapidOcrNet 2.0.0 — PP-OCRv5 over ONNX Runtime, Apache-2.0 code AND weights, ~12 MB bundled models, in-process, no GPU, no external network calls), implementing the existing `IDocumentOcrService` seam. Enablement requires TWO opt-ins: (1) GLOBAL `NoEgressOcr:Enabled=true` (Railway: `NoEgressOcr__Enabled=true`) on BOTH the API and the Worker — registers the real engine instead of `NoOpOcrService`; without it no models load and the default deploy is byte-for-byte unchanged (ships dormant + safe). (2) PER-ORG `Organisation.SelfHostedOcr=true` (DB column `self_hosted_ocr`, additive migration `AddSelfHostedOcrFlag`). For a no-egress org the WHOLE ingest/parse pipeline is no-egress — nothing sends that org's data to OpenAI: PDFs route to the deterministic parser with scanned/image-only pages OCR'd by RapidOcrNet (no OpenAI vision); AI mapping is gated — both line-level SKU suggestions and the magic auto-map field suggester (`OpenAiMappingService`), so unresolved lines/fields go to human review; email-body NLP extraction is gated (skipped); and the one-click AI schema-inference setup tool (`SchemaInferenceController`/`OpenAiSchemaInferencer`) is also gated (returns empty → org uses the manual mapping editor), so there is no remaining OpenAI touchpoint in that org's ingest/parse pipeline. Both Dockerfiles add `libgomp1` + `libfontconfig1` to the runtime stage (verified on the `aspnet:8.0` base). **Honest caveat:** even with self-hosted OCR, scanned/image-only lines are review-flagged (no text layer to verify numbers against) and illegible scans still fail with the existing "scanned or image-only" message — assisted, not silent. This is an ENTERPRISE / config capability enabled by an operator, NOT a self-serve UI toggle; the default prod deploy stays dormant (`NoEgressOcr:Enabled` unset → `NoOpOcrService`, no models loaded).
...
- **PDF strategy SUPERSEDED 2026-06-05** (see top section + `docs/superpowers/plans/2026-06-05-pdf-llm-extraction.md`): real Markit corpus is digital-text, not scanned, and brittle regex/templates fail on it. New direction: **text→LLM structured extraction is the PRIMARY PDF path** (the LLM IS the extractor on the digital text), vision LLM is the no-text fallback, self-hosted RapidOcrNet is the no-egress OCR path (now SHIPPED, opt-in via `NoEgressOcr:Enabled` + per-org `SelfHostedOcr`), and **Azure Document Intelligence is being removed**. The earlier "do not treat the LLM as the OCR engine" guidance is reversed for digital PDFs (no OCR step needed — the text layer is already exact).
...
**Status: Implemented for text-based purchase-order PDFs.** _(Superseded 2026-06-05: text→LLM structured extraction is now the PRIMARY PDF path and the regex `PdfOrderParser` below is the deterministic fallback; Azure Document Intelligence was removed. See the "PDF text→LLM extraction" section at the top of this file. Scanned/image-only PDFs are now supported via the Phase 2 AI vision fallback (PDFtoImage + SkiaSharp, no Dockerfile change; all scanned-PDF lines flagged for review); self-hosted no-egress OCR shipped as Phase 3 (RapidOcrNet, opt-in via `NoEgressOcr:Enabled` + per-org `SelfHostedOcr`).)_
...
| **F** | PDF ingestion (`PdfPig`) | **Implemented — text→LLM extraction is the PRIMARY path (`feat/pdf-llm-extraction` merging to `main`, 780 tests green); regex parser is fallback; Azure DI removed; scanned/image-only SHIPPED via Phase 2 AI vision (review-flagged, no Dockerfile change); self-hosted no-egress OCR SHIPPED as Phase 3 (RapidOcrNet, opt-in)** |
