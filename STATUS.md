...
> - **Wave 8 (pending):** final regression + reconcile stale docs (CLAUDE.md says "211 tests" → 988) + SPF/DKIM/DMARC on proculink.eu (Cloudflare full-write token provided in chat — held in memory, NOT committed) + consolidated runbook.
> - **Wave D (founder said DO-NOW, after blockers):** OrderService God-object split behind `IOrderService` facade; order status enum + transition table; Postgres RLS; typed-DTO layer; split `api-client.ts`; design-primitive page migrations (0/22 pages use `src/components/bridge/layout/*` + `UnifiedStatusBadge` yet — primitives + `docs/design-system/11-unified-page-rules.md` exist); EmailPolling indexed flag; R2/DB GDPR-delete; Redis only if >1 replica.
...
> - **⚠️ Broken uncommitted frontend WIP:** ~12 `(app)` page files mid-migration to design primitives have smart/curly quotes (`operations/health/page.tsx` fails `bun run build`). UNCOMMITTED → prod safe; do NOT `git add -A`. Complete properly before committing (see audit doc).
