...
**Phase 2 + Phase 3 IN PROGRESS this session:** scanned / image-only PDFs (no text layer) are NOT yet supported — they still fail with "This PDF looks scanned or image-only — we couldn't extract any text." **Phase 2** = vision-LLM fallback (PDFtoImage + SkiaSharp, both MIT; native deps Docker-verified before merge). **Phase 3** = self-hosted no-egress OCR (RapidOcrNet, Apache-2.0) with a per-org toggle.
