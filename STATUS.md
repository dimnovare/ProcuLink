...
- **Authenticated live-QA blocker:** production frontend loads Clerk from `clerk.proculink.eu` with a `pk_live...` key, while local `.env.local` is the old/test `golden-alpaca` instance. Pulling production Vercel env into a temporary file showed `CLERK_SECRET_KEY=""`, so Codex cannot create a disposable production Clerk user/session. Temporary env file was deleted. To close Group J, provide a real production Clerk secret key or a reusable production QA user/password, then run deployed upload -> preview -> review -> transform -> delivery QA against `https://proculink.eu` + `https://api.proculink.eu`.
