...
**SHIPPED 2026-06-20 (safe batch, `1312820`):** #17 "Retry delivery"→"Open to resend"; #16 dead-letter tile self-link→`/inbox?status=failed` + label "Out of retries". (#9 Rules already has a "catalog, not a gate" callout — left as-is. Also shipped MV-5 `339727a`.)

---

# Delivery / Idempotency Appendix (2026-06-20, round 2 — verified)

## NEW bugs
- **D-1 · HIGH (urgent) — redeliver/first-deliver can DOUBLE-SEND a PO to a real supplier.** `DispatchArtifactAsync` (`DeliveryService.cs:138`) sets `order.Status = Delivering` unconditionally and dispatches — **no atomic claim, no `[PerOrderDistributedMutex]`** (only the *retry* path `RetryDeliveryAsync`/`RetryDeliveryJob` has them). `DeliverOrderJob.cs:46` lacks the mutex; `OrdersController:1660` (Redeliver) + `OpsController:140` (Requeue) flip `delivery_failed→delivering` with a plain read-then-SaveChanges (no `WHERE status=delivery_failed`). Repro: double-click Redeliver, or Redeliver + Requeue, or Redeliver racing a scheduled `RetryDeliveryJob` → same PO POSTed twice. The deferred `(OrgId,OrderId,AttemptNumber)` unique index was never added; `delivery_attempts` index is non-unique (`ProcuLinkDbContext.cs:782`) → no DB backstop. **Fix:** give `DispatchArtifactAsync` the same atomic claim `RetryDeliveryAsync` uses + add `[PerOrderDistributedMutex]` to `DeliverOrderJob`; optionally add the unique index. **Approval-gated (sending).** Test: real-Postgres concurrency (2 parallel `DeliverOrderJob`, and `DeliverOrderJob` racing `RetryDeliveryJob`) → exactly one dispatch; redeliver on a `delivered` order = no-op.
- **D-2 · HIGH — two FE modules bypass the cold-auth fix → prod 401 P0 re-occurring.** `src/lib/api/mapping.ts:6-10` and `src/lib/api/connectors.ts:29-33` define a local `authHeader()` with NO `Clerk.loaded` wait (the canonical `core.ts:66-85` / `48cea6e` fix does). Hard-load the PO-mapping editor or `/operations/connectors` before Clerk loads → unauthenticated → 401 (mapping magic-auto-map silently dies; connectors throws). **Fix:** delete both locals, `import { authHeader } from "./core"`. **Touches auth → approval-gated per the safe-fix rule, but it's a pure consolidation onto the already-correct helper.** Test: `Clerk.loaded=false`→true after 100ms, assert the request carries `Authorization`.
- **D-3 · HIGH — `useMapperModel` save never invalidates `["order"]` (the concrete MV-1 mechanism).** `useMapperModel.ts:472` only `setQueryData(["mapper-override"])`; `OrderMappingOverrideService.UpsertAsync` never touches status. So a `ready_to_deliver` order keeps its pre-edit artifact and `confirmSend` redelivers it. `OutputMappingEditor` DOES invalidate `["order"]` — the per-field path is the inconsistent one. **Approval-gated (sending).**
- **D-4 · MEDIUM — override cache-key fragmentation:** `["mapper-override",…]` (written) vs `["mapping-override",id]` (read by workshop/preview) → stale "what we'll send" preview after a workbench save. **D-5 · MEDIUM — Inbox "Select all" caps at the current 25-row page** while chips show whole-account counts (not destructive — bulk action is redeliver, gated to redeliverable rows). Label "…on this page" or add server-side select-all.

## VERIFIED CLEAN (negative results — don't re-audit)
Org/tenant scoping (84-file sweep — every tenant query carries `OrgId==organisationId`; `AdminController` fail-closed allowlist; `BulkErase` org-scoped-first); transform idempotency (atomic `ready/transforming→transforming` claim + `Skipped`); `RetryDeliveryAsync` (atomic claim + stale-reclaim + cap + dead-letter in one tx); dispatchers HTTP/SFTP/S3/ERP (SSRF-guarded, header CR/LF guard, never throw, missing-config→`delivery_failed` not success); polling jobs (idempotent dedupe, SSRF, billing-gated, org-scoped); pagination (offset/limit + page/pageSize, clamp 100). Cold-auth `core.authHeader` is fixed (except D-2's two stragglers).
