...
- [—] **W1** OrderService decompose behind the `IOrderService` facade — **DEFERRED to a dedicated session.** 1618-LOC behind-facade move with shared private helpers (`MergeCanonicalJson`/`BuildAuditEvent`/`MapExtractedToParsed`/`GetAiMappingCandidates`) → a faithful split + adversarial review is ~a full session; the audit's own "pure risk, zero customer value pre-launch" item. Rushing it at the end of a 200+-step session is the exact new-bug risk the golden rule forbids. Ready-to-execute plan: `WAVE_D_BACKEND_REMAINING.md`.
