...
**Part 24 §1.1 bottlenecks** — [x] A pool ceiling · [x] B ListAsync retired→ListPaged · [x] C ingress idempotency · [x] D AI chunking · [x] E retention sweep (`DataRetentionService` + `DataRetentionSweepJob`, safe-by-default OFF; per-order R2 erase = W3; enablement is founder-env like every config-gated capability) · [x] F email-poller indexed flag (`eb24aa6`) · [—] G heavy-parse queue (marginal at pilot)
...
**Part 24 §2 DB** — [x] §2.3.2 AI-candidates sort index (`eb24aa6`) · [x] §2.3.3 email-flag index · [—] §2.3.4 invoices/ASN composite (frozen tables, no query) · [x] §2.4 canonical_json/ListAsync (retired) · [~] §2.4.2 BuyerName split (read-column-first pattern) · [—] §2.4.4 jsonb converter (deliberate) · [—] §2.5 Postgres RLS — DEFERRED (the audit ITSELF classifies it "redesign-later / post-revenue, NOT launch-blocking" §2.8/§2.5; needs a session-var connection interceptor + Hangfire/migration/cross-org-sweep role exemptions + a real-Postgres testcontainer two-org test harness the suite lacks; unset session var → zero rows would break sweeps/migrations — the highest prod-breaking risk remaining, so the no-new-bugs tiebreaker says "can't land green safely → drop"; app-level `.Where(OrgId==…)` already enforces 100% isolation per §2.5) · [—] §2.7.1 SchemaFingerprints rename (cosmetic/risky) · [x] §2.7.2 migrate-fail-loud (`5013fd8`); [—] phantom-migration cleanup (dangerous pre-launch)
...
**Part 28 code-quality** — [x] A1 dead/misleading CTAs (Wave 1 + DESADV→501 `d6c44ac`) · [x] B1 OrderService split (=W1, `28a72f4`) · [~] B5 stale docs (latest=correct; historical=labeled) · [—] B3/D5 typed-DTO layer (post-launch)

**UI/UX top-10** — [x] 1 pricing 6→3 · [x] 2 Distributor in-app path · [x] 3 .x12 import · [x] 4 wizard blue→green · [x] 5 wizard a11y · [x] 6 Admin-nav gating (`/api/admin/access` probe behind existing [AdminOnly] gate — BE `e41f6e6` / FE `ac16373`; UX-only, page+API still re-gate) · [~] 7 Document-Anatomy confidences · [x] 8 LimitBanner · [~] 9 mobile lineage · [~] 10/C1 landing claims · [—] C2 social proof (needs real data)
