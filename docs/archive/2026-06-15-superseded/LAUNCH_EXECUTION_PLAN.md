...
- [x] **W1** OrderService decomposed behind an internal facade — `OrderService.cs` **1618→124 lines** (same 12-dep ctor + `IOrderService`, so DI/Worker/tests untouched) delegating to `OrderIngestionService` / `OrderQueryService` / `OrderResolutionService` / `OrderTransformService` + `OrderServiceShared` (cross-cutting helpers, defined once). **Verbatim moves** — agent-implemented, then adversarially reviewed **SAFE-TO-MERGE** (every body byte-identical modulo 4 mechanical helper-call rewrites; shared DbContext preserved by reference; integration-trigger awaits intact). **1028 tests green** (51 OrderService tests pass). NOTE: the f-2 "Worker stops mirroring DI" win is a further step (move the orchestrator to `Core` + DI-register the sub-services) — deliberately left out to keep this change zero-risk.
- [x] **UI/UX #6** Admin-nav gating — new bodyless `GET /api/admin/access` (reuses the class-level `[AdminOnly]` gate → 204 admin / 403 non-admin; no DB hit; allowlist never serialized) + `BridgeSidebar` filters the Admin link via a `checkAdminAccess` probe (gated on `queryEnabled`, 5-min staleTime, no retry, default-hidden while loading, `true` in mock for the demo). UX-only — the `/admin` page + every `/api/admin` endpoint still re-gate server-side, so it can only show/hide a link, never grant access. BE `e41f6e6` / FE `ac16373`. 17 AdminControllerTests green + FE build clean.
- [—] **§2.5** Postgres RLS (defence-in-depth) — **DEFERRED to a dedicated session.** Needs a connection-level session-var interceptor (`SET app.org_id` per request) + a Hangfire/migration/sweep role exemption + a real-Postgres two-org test harness (EF InMemory cannot test RLS); a wrong policy or unset var silently zeroes queries / blocks writes. Highest-risk item; audit "redesign-later". App-level org scoping already enforces isolation. Plan: `WAVE_D_BACKEND_REMAINING.md`.
