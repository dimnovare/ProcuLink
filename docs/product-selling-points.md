...
- **Scanned-PDF support requires sending page images to OpenAI.** Scanned / image-only PDFs are supported via the AI vision fallback, but it rasterizes pages and sends the images to OpenAI (same EU-residency/DPA/zero-retention considerations as the text path), and every line is flagged for human review. A self-hosted no-egress OCR engine (RapidOcrNet) for customers who cannot send anything to OpenAI is **planned, not built** (Phase 3) — do not promise on-prem / "no data leaves your environment" OCR yet. Illegible scans still fail with "This PDF looks scanned or image-only — we couldn't extract any text." No Azure provider — Azure Document Intelligence was removed.
