# Group L Expanded — Magic Mapping + Per-Industry Templates + Standards Visibility + Help Completion (Implementation Plan)

> **⚠️ SUPERSEDED SECTIONS (2026-05-29):** The dual-persona / default-vs-expert mode workstream below is **DROPPED**. Successful B2B SaaS products ship ONE great experience with smart defaults + progressive disclosure + a Command Palette (Linear, Stripe, Notion, Vercel, Railway), not user-mode toggles. The `useViewMode()` hook, `<ViewModeToggle />` component, `proculink_view_mode_v1` localStorage key, and any "default mode" / "expert mode" branching are not to be implemented. Power-user affordances (standards mappings inline, raw-view, hotkeys, density) still ship — they surface through the existing Command Palette (Cmd+K), per-table column selectors, and info popovers. The remaining workstreams in this plan (**magic mapping preview**, **per-industry templates**, **standards-visibility affordances**, **in-app help completion**, **analytics funnel completion**, **trust pack live links**) are still in scope and should be executed as written EXCEPT for any "render in expert mode" or `useIsExpert()` references. Treat every "expert mode" reference as "render via info popover or Command Palette entry" and every "default mode" reference as "the only mode". See `CLAUDE.md` "Coding conventions → Product-level rules → One great experience rule" and `docs/design-system/00-agent-quick-brief.md` "One Great UX (Phase 6+)" for the locked replacement direction.
