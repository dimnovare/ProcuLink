...
- **Phase 2:** vision fallback for no-text PDFs — rasterize via **PDFtoImage (MIT) + SkiaSharp (MIT)** (NuGet native assets, Debian-OK; **never** Ghostscript/Magick.NET/poppler — AGPL/GPL), send `ChatMessageContentPart.CreateImagePart(BinaryData, "image/png", ChatImageDetailLevel.High)`.
- **Phase 3:** **RapidOcrNet** (Apache-2.0 code+weights, in-process, no GPU) as `IDocumentOcrService` for no-egress orgs; per-org toggle.
- **Phase 4 (optional, separate):** enrich the canonical model (supplier name, totals, tax, payment terms, per-line delivery date) — needs `ParsedOrder`/entity changes + EF migration; and PO-vs-invoice doc-type detection (real invoices currently arrive on the PO path with no classifier). Out of scope for the first ship; note it.
