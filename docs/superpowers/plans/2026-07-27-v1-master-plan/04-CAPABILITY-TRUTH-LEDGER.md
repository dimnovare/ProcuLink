...
| `revision-pinning` | ✓ | ✓ | ~ | ✓ | **✓ flag ON** | `Connections__RevisionAuthority = true` on BOTH Railway services, verified 2026-07-27 and re-verified 2026-07-31. Reproducibility is LIVE. The audit's "inert in production" P0 is **refuted**. WP-21 (BE #98) now PROVES a pinned order does not re-route after a live config edit: `PinnedOrderDoesNotRerouteAfterConfigEditPostgresTests` publishes v1, pins an order, edits the live endpoint, republishes (archiving v1), and asserts the pinned order still dispatches to v1 while an unpinned one takes the edit — asserting the DIFFERENCE, with a flag-off companion proving the difference is the flag. Remaining gap is OBSERVATIONAL, not behavioural: the live production smoke is unexecuted and is a founder action (`docs/ops/revision-authority-production-smoke.md`) |
