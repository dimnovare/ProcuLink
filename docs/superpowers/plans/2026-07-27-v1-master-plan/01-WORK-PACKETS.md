...
**Do:** rewrite residency to the checkable version — order files and the database are EU-region (Neon, R2, Railway europe-west4); named US subprocessors process specific categories under SCCs, listed on `/subprocessors`. Keep the "EU" stat only if it links to that explanation. `/customers`: an honest "no public references yet" placeholder, or a real dated engagement.
