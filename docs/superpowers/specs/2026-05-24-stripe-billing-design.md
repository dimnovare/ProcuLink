...
Add Stripe-powered subscription billing to ProcuLink. Five tiers — Sandbox (free), Growth, Operations, Integration, Enterprise — designed around Integration/Enterprise as the primary revenue engine. The Sandbox and Growth tiers serve as qualified lead generation; the real ARR target (€3M within 3 years) comes from 30–100 Integration/Enterprise accounts.
...
| Feature | Sandbox | Growth | Operations | Integration | Enterprise |
|---|---|---|---|---|---|
| CSV / XLSX | ✓ | ✓ | ✓ | ✓ | ✓ |
| XML | — | ✓ | ✓ | ✓ | ✓ |
| PDF | — | ✓ | ✓ | ✓ | ✓ |
...
Existing `plan` string column stays. Updated by webhook events. Defaults to `"sandbox"`.
...
{
    [PlanConstants.Sandbox]     = new(20,    1),
...
- CTA row: "Upgrade to Growth", "Upgrade to Operations", "Upgrade to Integration" — three buttons, each posts `{ plan }` to `/api/billing/checkout` and redirects
...
> "You've reached your [N]-order monthly limit. [Upgrade your plan →]"
...
Sandbox and Growth tiers are acquisition funnels, not revenue targets. The product, onboarding, and sales motion should all optimise for converting Sandbox → Growth → Operations trials into Integration contracts.
