...
- **Inbound too**: `{slug}@orders.proculink.eu` → Postmark inbound → webhook. Docs disagree on one
  ...
  `Cloudflare Email Routing MX → Postmark inbound`, while the live-proof at
  `docs/prompts/2026-07-24-open-queue-handover.md:251` records
  `orders.proculink.eu MX → inbound.postmarkapp.com` directly. Either way the mail lands in
...
| E8 | **OpenAI EU-residency project and ZDR are NOT provisioned** (dashboard-verified). The **DPA is signed**. | doc `docs/prompts/2026-07-24-open-queue-handover.md:461–464`; `STATUS.md:573, 956` |
