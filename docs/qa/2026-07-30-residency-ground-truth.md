...
(the local `project-proculink` checkout). Ambiguous cases are prefixed `FE`/`BE`.
...
| **FA-3** | U1 | Railway dashboard → project → the API service → **Settings → Region**; repeat for the Worker service. Record the full identifier (expect `europe-west4-drams3a`, not `europe-west4`). Confirm **both** services, not just the API — a Worker in a different region would be invisible today. | Yes |
