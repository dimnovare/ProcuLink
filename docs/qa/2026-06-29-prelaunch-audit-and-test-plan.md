...
| P3 | Test-fire uses the LIVE supplier delivery config (DeliveryService.TestFireAsync), NOT the pinned connection revision used by real order delivery (ResolveEffectiveDeliveryConfigAsync). When Connections:RevisionAuthority is on, a successful test-fire can validate a different (live-edited) channel than the one a pinned order will actually deliver over, giving a false-positive 'it works'. | `ProcuLink.Infrastructure/Services/DeliveryService.cs:370 (TestFireAsync) vs :323 (ResolveEffectiveDeliveryConfigAsync)` |
