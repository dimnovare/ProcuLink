...
### Email/IMAP
- **Postmark account + inbound domain with MX + parse webhook pointed at /api/inbound-email/postmark, and Inbound:Postmark:WebhookToken set** — Required to test hosted inbound-email PO ingestion and its token auth. Since 2026-08 the tenant is selected by the RECIPIENT address (org_inbound_addresses), not the org slug, so the org under test also needs Security:ApiKeyHashSecret configured and an issued inbound address — GET /api/settings/inbound-email mints the primary one on first read and shows it.
...
- Inbound:Postmark:WebhookToken + Security:ApiKeyHashSecret + an issued inbound address for the target org (GET /api/settings/inbound-email) + ability to POST Postmark-shaped JSON for inbound-email tests; the recipient address is the per-tenant credential, so a payload whose To: address was never issued names no org at all — external-dep: Postmark
...
- **[EMAIL-HOST-1] Supplier emails a PO to the org's inbound address** _(happy · env: external-dep)_
  - Steps: (1) Read the org's inbound address from GET /api/settings/inbound-email (2) Send an email with a CSV attachment to it — `{token}@orders.proculink.eu` (3) Postmark POSTs the parsed MIME to the webhook
  ...
  - Prereq: Postmark inbound configured (MX + parse webhook → /api/inbound-email/postmark), Inbound:Postmark:WebhookToken set, Security:ApiKeyHashSecret set, the org has an ISSUED inbound address (no address ⇒ no mail routes to that org, whatever the slug is), and the org has a default/first supplier. Founder must supply the Postmark account + domain.
- **[EMAIL-HOST-2] Inbound email to an unknown tenant / blocked org / no supplier** _(error · env: external-dep)_
  - Steps: (1) Send to an address that was never issued — a random token at the inbound domain (2) Send to a valid org that is read-only/trial-expired (3) Send to a valid org that has zero suppliers
  ...
  - **CORRECTION (2026-08-13):** the tenant is no longer selected by the org slug. BE PR #190 moved selection to `org_inbound_addresses`, where the recipient address is a per-tenant credential (128-bit token, HMAC-hashed for lookup, AES-GCM encrypted for display) resolved by `IInboundAddressService`; the `Inbound:Postmark:TenantMapping` config fallback is deleted. For these steps: (a) an org needs an ISSUED inbound address before any mail routes to it — `GET /api/settings/inbound-email` mints the primary one on first read, `POST /api/settings/inbound-email/rotate` and `POST /api/settings/inbound-email/{id}/revoke` are org-admin-only; (b) existing slugs were backfilled as expiring `legacy_slug` addresses (90 days, `Inbound:LegacyAddressGraceDays`), so `{slug}@orders.proculink.eu` keeps working until it expires and then stops; (c) an unissued, revoked or expired address behaves exactly like the old unknown-slug case — **200** `{status:"ignored"}`, nothing ingested, no retry — and that branch does not echo the address into the log line or the response body, because a mistyped credential is still a credential (a recipient that is not shaped like an inbound address at all is refused one step earlier, and that branch does name it); (d) one NEW transient case: with `Security:ApiKeyHashSecret` unset NO address can be recognised, so the webhook answers **422** to keep Postmark re-delivering rather than settling real orders as handled.
  - Prereq: Postmark inbound configured, and an issued inbound address for each org under test.
...
- Hosted inbound email: Postmark inbound MX + parse webhook pointed at /api/inbound-email/postmark and Inbound:Postmark:WebhookToken set (founder-supplied Postmark account + domain), plus Security:ApiKeyHashSecret set and an issued inbound address for each org under test (GET /api/settings/inbound-email) — the recipient address selects the tenant, the slug no longer does. Optional overrides: Inbound:Postmark:InboundDomain (preferred local-part scheme `{token}@orders.proculink.eu`, one MX), Inbound:Postmark:HostSuffix (legacy subdomain scheme `orders@{token}.proculink.eu`, needs a wildcard MX), Inbound:LegacyAddressGraceDays (lifetime of the backfilled legacy slug addresses). `Inbound:Postmark:TenantMapping` no longer exists — it was deleted with the slug-routing path.
