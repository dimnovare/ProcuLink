...
- **Group F PDF ingestion — Phase 1 (text→LLM) + Phase 2 (vision for scanned PDFs) SHIPPED 2026-06-05** (`feat/pdf-llm-extraction` merging to `main`; 780 backend tests green; live-verified). Spec: `docs/superpowers/plans/2026-06-05-pdf-llm-extraction.md`; benchmark-proven on 22 real Markit docs.
  ...
  - **Still pending (NOT built — do not claim as available):** self-hosted no-egress OCR (**RapidOcrNet**, Apache-2.0) is **Phase 3** — so "no data leaves your environment" OCR is NOT available yet. Supplier/totals/tax/per-line-delivery-date enrichment + a PO-vs-invoice classifier shipped as **Phase 4**.
