...
- [x] Phase 3 self-hosted no-egress OCR (`RapidOcrDocumentOcrService` / RapidOcrNet 2.0.0, Apache-2.0 code+weights, in-process, no external calls) — opt-in via GLOBAL `NoEgressOcr:Enabled` + PER-ORG `Organisation.SelfHostedOcr` (`AddSelfHostedOcrFlag` migration); whole ingest/parse pipeline no-egress for such orgs (AI vision/SKU mapping/email-body NLP/schema-inference all gated); `libgomp1` + `libfontconfig1` added to both Dockerfile runtime stages; default deploy dormant + safe
