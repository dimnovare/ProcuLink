...
else
{
    authBuilder.AddJwtBearer(options =>
    {
        ...
        // Keep the raw "sub" claim: without MapInboundClaims=false it is renamed to
        // ClaimTypes.NameIdentifier before reaching HttpContext.User, which would break
        // the analytics userId (and any sub-based lookup).
...
// succeeds immediately. We use ApplicationStarted so the scope is created
// inside a running host, and Task.Run so the callback returns at once.
//
// The token is ApplicationStopping: the retry backoff must not keep a
// fire-and-forget task — and its eventual write to the process-global
// MigrationReadiness flag — alive after the host it belongs to is gone.
// See MigrationBootstrap for the loop, the fail-loud path, and the
// non-relational skip.
app.Lifetime.ApplicationStarted.Register(() =>
{
    _ = Task.Run(
        () => MigrationBootstrap.RunAsync(
            app.Services, app.Configuration, app.Lifetime.ApplicationStopping),
